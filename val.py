...
def run(
    data,
    weights=None, 
    weights_T=None, # model.pt path(s)
    ...
    if training:  # called by train.py
        ...
        teacher = DetectMultiBackend(weights_T, device=device, dnn=dnn, data=data, fp16=half)
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[1]:
            ...
            with torch.no_grad():
                teacher_pred, teacher_logits = teacher(im)
        ...
        if compute_loss:
            ...
            def calculate_kl_loss(teacher_logits, student_logits, temperature=1.0):
                ...
                for t_logit, s_logit in zip(teacher_logits, student_logits):
                    ...
                    kl_loss += F.kl_div(t_prob, s_prob, reduction='batchmean')

                return kl_loss

            teacher_logits = teacher_logits[:len(train_out)]

            kl_loss = calculate_kl_loss(teacher_logits, train_out, temperature=1.0)
...
def parse_opt():
    ...
    parser.add_argument("--weights_T", nargs="+", type=str, default=ROOT / "yolov5x6.pt", help="model path(s)")
